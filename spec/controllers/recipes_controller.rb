...
RSpec.describe RecipesController, type: :controller do
  describe 'GET #index' do
    ...
    context 'when user is authenticated' do
      ...
      it 'assigns all recipes to @recipes' do
        recipe1 = Recipe.create!(name: 'Recipe 1', user: user, preparation_time: 30, cooking_time: 45)
        recipe2 = Recipe.create!(name: 'Recipe 2', user: user, preparation_time: 20, cooking_time: 60)
  ...
  describe 'GET #show' do
    context 'when user is authenticated' do
      let(:user) { User.create(name: 'Test User', email: 'test@example.com', password: 'password', confirmed_at: Time.now) }
      let(:recipe) { Recipe.create!(name: 'Recipe 2', user: user, preparation_time: 20, cooking_time: 60)}
  ...
  describe 'PATCH #update_public_status' do
    ...
    context 'when user is authenticated' do
      let(:user) { User.create(name: 'Test User', email: 'test@example.com', password: 'password', confirmed_at: Time.now) }
      let(:recipe) { Recipe.create(name: 'Recipe',preparation_time: 20, cooking_time: 60, user: user, public: false) }
  ...
  describe 'GET #public_recipes' do
    ...
    it 'assigns public recipes to @recipes' do
      ...
      recipe3 = Recipe.create(name: 'Recipe 3', user_id: user.id, preparation_time: 20, cooking_time: 60, public: false)
