...
RSpec.describe 'recipes/index', type: :view do
  ...
  before do
    ...
    assign(:recipes, [
      Recipe.create(name: 'Recipe 1', user: user, preparation_time: 30, cooking_time: 45, description: 'Description 1'),
      Recipe.create(name: 'Recipe 2', user: user, preparation_time: 20, cooking_time: 60, description: 'Description 2')
    ])
    render
  end

  it 'displays a list of recipes' do
    expect(rendered).to have_selector('.recipe', count: 2)
  end

  it 'renders the recipe details correctly' do
    expect(rendered).to have_selector('.recipe h2 a', text: 'Recipe 1')
    expect(rendered).to have_selector('.recipe p', text: 'Description 1')
    expect(rendered).to have_selector('.recipe h2 a', text: 'Recipe 2')
    expect(rendered).to have_selector('.recipe p', text: 'Description 2')
  end
