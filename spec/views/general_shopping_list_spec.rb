...
RSpec.describe 'recipes/general_shopping_list', type: :view do
  ...
  let(:recipe) { Recipe.create(name: 'Recipe 1', user: user, preparation_time: 30, cooking_time: 45, public: false) }
  let(:food) { Food.create!(name: 'Food 1', user: user, measurement_unit: 'grams', price: 2.5, quantity: 100) }
  let!(:food_recipe) { FoodRecipe.create!(recipe: recipe, food: food, quantity: 200) }
  ...
  it 'displays the food ingredients table' do
    expect(rendered).to have_selector('table.ingredient-table')
